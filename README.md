...
The server supports **two MCP transports** so both modern and legacy clients (e.g. Home Assistant) work:

| Transport        | Use case              | Endpoints |
|------------------|------------------------|-----------|
| **Streamable HTTP** | New clients (recommended) | **POST** `http://&lt;host&gt;:3000/mcp` (or 3100 standalone). Session reuse: include **Mcp-Session-Id** header; **GET** /mcp for SSE stream, **DELETE** /mcp to end session. |
| **HTTP+SSE**     | Home Assistant, legacy | **GET** `http://&lt;host&gt;:3000/mcp/sse` to open the SSE stream (server returns an `endpoint` event with the messages URL). Then **POST** to that URL (e.g. `http://&lt;host&gt;:3000/mcp/messages?sessionId=&lt;id&gt;`) to send messages. |
